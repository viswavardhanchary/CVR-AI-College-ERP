...
export function TeacherHeader({ onToggleSidebar, teacherData, handleMouseEnter, handleMouseLeave, collegeData }) {
        ...
        <header className="bg-white shadow-sm h-16 flex items-center justify-between px-4 lg:px-6 shrink-0 z-50 border-b border-gray-200 relative">
            ...
            <div className="flex items-center gap-2">
                <div className="hidden md:flex items-center gap-4">
                    <button onMouseEnter={onMessagesEnter} onMouseLeave={handleMouseLeave} className="relative p-2 text-gray-500 hover:bg-gray-100 rounded-full">
                        <MessageSquare className="w-5 h-5 stroke-2" />
                        <span className="absolute top-1 right-1 w-4 h-4 text-[10px] font-bold text-white bg-red-500 rounded-full border-2 border-white flex items-center justify-center">0</span>
                    </button>
                    <button onMouseEnter={onNotificationsEnter} onMouseLeave={handleMouseLeave} className="relative p-2 text-gray-500 hover:bg-gray-100 rounded-full">
                        <Bell className="w-5 h-5 stroke-2" />
                        <span className="absolute top-1 right-1 w-4 h-4 text-[10px] font-bold text-white bg-red-500 rounded-full border-2 border-white flex items-center justify-center">1</span>
                    </button>
                    <div onMouseEnter={onProfileEnter} onMouseLeave={handleMouseLeave} className="w-10 h-10 rounded-full border-2 border-blue-100 overflow-hidden cursor-pointer hover:border-blue-300">
