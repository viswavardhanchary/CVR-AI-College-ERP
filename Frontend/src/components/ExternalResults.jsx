...
export function ExternalResults() {
    ...
    if (state.loading) {
        return <Loading title="Loading external results..." />;
    }
        ...
        <div className="bg-gray-50 font-body min-h-screen text-gray-800">
            <header className="bg-[#051F3E] shadow-md h-16 flex items-center px-4 sm:px-8 sticky top-0 z-40">
                <div className="flex items-center gap-4 w-full max-w-5xl">
                    ...
                    <h1 className="font-heading font-bold text-lg text-white">External Semester Results</h1>
                </div>
            </header>
            <main className="p-4 sm:p-8 flex justify-center pb-20">
                <div className="w-full max-w-5xl">
                    <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 mt-4">
                        <div>
                            <h2 className="text-2xl font-bold font-heading text-[#051F3E] mb-1">External Semester Results</h2>
                            <p className="text-sm font-medium text-gray-500">
                                {state.metadata ? `Year ${state.metadata.fromYear} - ${state.metadata.toYear} · Semester ${state.metadata.semester} · ${state.metadata.examMethod ? state.metadata.examMethod : 'N/A'}` : 'Filtered external results'}
                            </p>
                        </div>
                        <div className="bg-blue-50 border border-blue-100 px-4 py-2 rounded-xl inline-flex items-center gap-2 shadow-sm">
                            <Award className="w-4 h-4 text-[#051F3E]" />
                            <span className="text-sm font-semibold text-[#051F3E]">External Results</span>
                        </div>
                    </div>
                    {state.error ? (
                        ...
                        <div className="rounded-2xl bg-white p-12 text-center shadow-sm border border-gray-200">
                            <p className="text-gray-600">No external results found for the selected filters.</p>
                        </div>
                    ) : (
