...
export function TeacherDashboard() {
    ...
    useEffect(() => {
        // Added the missing resize event listeners
    ...
    const getDetails = async () => {
        ...
        if (response.status) {
            ...
            setTeacher(response.user);
            setCollegeData(response.college);
            setFilters({ ...filters, loading: false });
        } else if (response.error) {
            setFilters({
                ...filters, loading: false,
                toast: { message: response.message, status: true, type: 'error' }, refresh: true
            });
        } else {
            setFilters({
                ...filters, loading: false,
    ...
    function handleMouseLeave() { setTooltip(prev => ({ ...prev, visible: false })); }
            ...
            {!filters.loading && !filters.refresh && !filters.navigate.status &&
                <div className="bg-gray-50 h-screen overflow-hidden font-body relative">
                    ...
                    <div className="flex h-full">
                        ...
                        {/* ml-0 applied everywhere below lg (1024px) so it hovers */}
