...
export function AdminDashboard() {
    ...
    const getDetails = async () => {
        ...
        if (response.status) {
            if (response.redirectURL) navigate(response.redirectURL, { replace: true });
    ...
    function expandSidebar() {
        if (uiState.isSidebarCollapsed) setUiState(prev => ({ ...prev, isSidebarCollapsed: false }));
    }
    ...
    function onClose() {
        setFilters({ ...filters, toast: { message: "", status: false, type: '' } });
    }

    return (
        <div className="">
            {filters.toast.status && <div className="mb-3"><Toast message={filters.toast.message} type={filters.toast.type} onClose={onClose} /></div>}
            {filters.refresh && <div className="mb-3">Refresh....</div>}
            {filters.navigate.status && <div className="mb-3">Redirecting to <Link to={filters.navigate.url} className="underline text-blue-600">{filters.navigate.url}</Link> in a second</div>}
            ...
            {!filters.loading && !filters.refresh && !filters.navigate.status &&
                <div className="bg-gray-50 h-screen overflow-hidden font-body relative">
                    <AdminHeader adminData={admin} handleSidebarToggle={handleSidebarToggle} handleMouseEnter={handleMouseEnter} handleMouseLeave={handleMouseLeave} collegeData={collegeData}/>
                    ...
                    <div className="flex h-full">
                        ...
                        {/* ml-0 applied everywhere below lg (1024px) so it hovers. lg:ml-20 or lg:ml-60 take space on desktop */}
